...
function Counter() {
  ...
  useEffect(() => {
    console.log("[Function] useEffect [value]: 컴포넌트가 마운트 될 때, + value가 변경되면,")
    ...
    return () => {
      console.log("[Function] useEffect return [value]: 새로 useEffect를 수행하기 전에,")
      document.body.removeEventListener("click", eventHandler)
    }
  }, [value]) // value 값이 변경되는 순간에 useEffect 함수를 실행하겠다!
  // Increase value 버튼을 누르면 Render Counter! -> 클린업 함수 실행 -> 마운트 실행행
